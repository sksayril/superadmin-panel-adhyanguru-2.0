...
export default function Chapters() {
  ...
  }, [token, subjectId, filterActive]);

  const fetchSubject = async () => {
    if (!token || !subjectId) return;
    setLoading(true);
    try {
      const response = await api.getSubjectById(token, subjectId);
      if (response.success) {
        setSubject(response.data);
      }
    } catch (error: any) {
      showToast(error.message || 'Failed to fetch subject', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchChapters = async () => {
    ...
    setLoading(true);
    try {
      const response = await api.getChaptersBySubject(token, subjectId, filterActive);
      if (response.success) {
        setChapters(response.data);
      }
    } catch (error: any) {
      showToast(error.message || 'Failed to fetch chapters', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateChapter = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(error.message || 'Failed to create chapter', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpdateChapter = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(error.message || 'Failed to update chapter', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      await api.deleteChapter(token, deleteTarget.id);
  ...
  const handleEdit = async (chapter: Chapter) => {
    ...
    setChapterForm({
      ...
      order: chapter.order,
      textContent: chapter.content.text || '',
      pdf: null,
      video: null,
  ...
  const handleViewDetails = async (id: string) => {
    ...
    setLoading(true);
    try {
      const response = await api.getChapterById(token, id);
      if (response.success) {
        setSelectedChapter(response.data);
        setShowDetails(true);
      }
    } catch (error: any) {
      showToast(error.message || 'Failed to fetch chapter details', 'error');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div>
            ...
            {subject && (
              <p className="text-gray-600">
                Managing chapters for: <span className="font-semibold">{subject.title}</span>
              </p>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          ...
          <div className="col-span-full text-center py-12 text-gray-500">
            ...
            <p className="text-sm mt-2">This subject doesn't have any chapters yet.</p>
          </div>
        ) : (
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <div>
              ...
              {existingChapter?.content.pdf && !form.pdf && (
                <div className="mb-2">
                  <a
                    href={existingChapter.content.pdf.url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-sky-600 hover:underline flex items-center gap-2"
                  >
                    <File className="w-4 h-4" />
                    {existingChapter.content.pdf.fileName}
                  </a>
                </div>
              )}
              <input
            ...
            <div>
              ...
              {existingChapter?.content.video && !form.video && (
                <div className="mb-2">
                  <a
                    href={existingChapter.content.video.url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-sky-600 hover:underline flex items-center gap-2"
                  >
                    <Video className="w-4 h-4" />
                    {existingChapter.content.video.fileName}
                  </a>
                </div>
              )}
              <input
            ...
            {chapter.content.video && (
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <a
                    href={chapter.content.video.url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-sky-600 hover:underline flex items-center gap-2 text-sm"
                  >
                    <Video className="w-4 h-4" />
                    {chapter.content.video.fileName}
                    <span className="text-xs text-gray-500">(Open in new tab)</span>
                  </a>
                </div>
                <div className="bg-gray-900 rounded-lg border border-gray-300 overflow-hidden">
                  <video
                    src={chapter.content.video.url}
                    controls
                    className="w-full"
